...
const backgroundInfoSchema: ObjectSchema = Joi.object().keys({
...
const changePasswordSchema: ObjectSchema = Joi.object().keys({
  ...
  newPassword: Joi.string().required().min(4).messages({
    'string.base': 'Password should be a type of string',
    'string.min': 'Password must have a minimum length of {#limit}',
    'string.empty': 'Password is a required field'
  }),
...
export { backgroundInfoSchema, socialLinksSchema, changePasswordSchema, notificationSettingsSchema };
