...
export class Update {
  ...
  public async comment(req: Request, res: Response): Promise<void> {
    ...
    const commentAfterBeingUpdated: ICommentDocument | null = await commentCache.updateACommentInCache(commentId, updatedComment);
    commentQueue.addCommentJob('updateACommentInDB', { key: commentId, value: updatedComment });
    const commentAfter: ICommentDocument = commentAfterBeingUpdated
      ? commentAfterBeingUpdated
      : await commentService.getSingleComment(commentId);
    res.status(STATUS_CODE.OK).json({ message: 'updated comment sucessfully ', updatedComment: commentAfter });
  }
}
