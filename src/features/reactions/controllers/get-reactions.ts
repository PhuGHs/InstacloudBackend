...
export class Get {
  public async postReactions(req: Request, res: Response): Promise<void> {
    ...
    const reactions: [IReactionDocument[], number] = cachedReaction[0].length ? cachedReaction :
  ...
  public async singlePostReaction(req: Request, res: Response): Promise<void> {
    ...
    const reaction: IReactionDocument = cachedReaction ? cachedReaction :
    await reactionService.getSingleReaction({ postId: new mongoose.Types.ObjectId(postId), username: req.currentUser!.username }, { createdAt: -1});
  ...
  public async singleCommentReaction(req: Request, res: Response): Promise<void> {
    ...
    const reaction: IReactionDocument = cachedReaction ? cachedReaction :
    await reactionService.getSingleReaction({ postId: new mongoose.Types.ObjectId(commentId), username: req.currentUser!.username }, { createdAt: -1});
