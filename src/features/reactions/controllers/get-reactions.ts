...
export class Get {
  public async postReactions(req: Request, res: Response): Promise<void> {
    ...
    const cachedReaction: IReactionDocument[] = await reactionCache.getPostReaction(postId, req.currentUser!.username);
    const reactions: IReactionDocument[] = cachedReaction.length
      ? cachedReaction
  ...
  public async commentReactions(req: Request, res: Response): Promise<void> {
    ...
    const cachedReaction: IReactionDocument[] = await reactionCache.getCommentReaction(commentId, req.currentUser!.username);
    const reactions: IReactionDocument[] = cachedReaction.length
      ? cachedReaction
